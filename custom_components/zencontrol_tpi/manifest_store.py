...
async def load_entities_from_manifest(hub: ZenHub, manifest: DiscoveryManifest) -> bool:
    ...
    hub.lights.sort(key=lambda light: light.address.number)
    hub.fans.sort(key=lambda fan: fan.address.number)
    hub.blinds.sort(key=lambda blind: blind.address.number)
    hub.groups.sort(key=lambda group: group.address.number)
    hub.buttons.sort(key=lambda button: (button.instance.address.number, button.instance.number))
    hub.motion_sensors.sort(key=lambda motion_sensor: (motion_sensor.instance.address.number, motion_sensor.instance.number))
    hub.absolute_inputs.sort(key=lambda absolute_input: (absolute_input.instance.address.number, absolute_input.instance.number))
    hub.profiles.sort(key=lambda profile: (profile.ctrl.name, profile.number))
    return needs_save
